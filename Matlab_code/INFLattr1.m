...
for i=1:count(1)
    for j=1:2
        xvec(i,j)=0.032+(i-1)*0.005 + (j-1)*0.005;
        yvec(i,j)=0.532;
    end
end
for i=count(1)+1:count(2)
    for j=1:2
        xvec(i,j)=0.04+count(2)*0.01 -((i-1-count(1))*0.012 + (j-1)*0.012);
        yvec(i,j)=0.532;
    end
end
for i=count(2)+1:count(3)
    for j=1:2
        xvec(i,j)=0.060+count(2)*0.01 + ((i-1-count(2))*0.013 + (j-1)*0.0013);
        yvec(i,j)=0.532;
    end
end
for i=count(3)+1:NNN-15
    for j=1:2
        xvec(i,j)=0.09+NNN*0.009 -((i-1-count(3))*0.0082 + (j-1)*0.0082);
        yvec(i,j)=0.532;
    end
end
...
%nexttile ([3 1])
%hold on
%plot(inp, Eii,'Color','k','LineStyle','--')
%plot(inp, inp,'Color','k','LineStyle','-')
%xlim([-0.015 0.25])
%ylim([-0.015 0.25])
%hold off
iinp=inp*100;
...
ylabel('E(\pi^{t}|\pi^{t-1}) - \pi^{t-1}','FontSize',17)
txt=['\pi^{t-1}=',num2str(EEii(count(1)),'%2.2f'),'%'];
text(iinp(count(1))+0.14,-0.03,txt,'FontSize',17);
txt1=['\pi^{t-1}=',num2str(EEii(count(2)),'%2.2f'),'%'];
text(iinp(count(2))+0.04,-0.03,txt1,'FontSize',17);
txt2=['\pi^{t-1}=',num2str(EEii(count(3)),'%2.2f'),'%'];
text(iinp(count(3))-0.8,-0.03,txt2,'FontSize',17);
xlim([0 27])
ylim([-0.7 0.7])
