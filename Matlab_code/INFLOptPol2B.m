...
%IND=[0.025 0.025];
IND=[0.0025 0.075]; % Optimal two bin monetary policy
Inst=0.04;
...
for k=1:1
    ...
    while Q < 1
        ...
        hew=zeros(Block,N,1);hewl=zeros(Block,N,1);
        hw=zeros(Block,N,1);hwl=zeros(Block,N,1);
        mp1=zeros(Block,1);mp1l=zeros(Block,1);mhew=zeros(Block,1);mhewl=zeros(Block,1);
        mhw=zeros(Block,1);UE=zeros(Block,1);    
        I(1)=IND(1);
        ...
        for t=1:Block
            ...
            for m=3:TT
                ...
                for g=1:N
                    ...
                    w(m,g)=(c/ap(m,g))*ew(m-1,g)*(infl(m-1));
            ...
            NCw(t)=lam2*I(t)+lam*in2(t);%+lam*(1+in2(t))^2;
            for i=1:NNN
                for j=1:NNN
                    if t>1
...
for i=count(1)+1:NNN
    for j=1:2
        xvec(i,j)=0.08+NNN*0.009 -((i-1-count(1))*0.01 + (j-1)*0.01);
        yvec(i,j)=0.532;
    end
end
iinp=inp*100;
...
%hold on
%box on
%plot(inp*100,DEi*100,'Color','k','LineStyle','--')
%plot(inp*100,DIi*100,'Color','k')
%for i=1:(NNN)/3
%    annotation('textarrow',xvec(i*3,:),yvec(i*3,:))
%end 
%xlabel('Inflation in current period (\pi^{t})','FontSize',17)
%ylabel('E(\pi^{t+1}|\pi^{t}) - \pi^{t}')
%txt=['\pi^{t}=',num2str(EEii(count(1)),'%2.2f'),'%'];
